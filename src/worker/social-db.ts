...
export class PgSocialDb implements SocialDb {
  constructor(private readonly pool: Pool, private readonly realm: string) {}
  ...
  async findCharacterByName(name: string): Promise<CharInfo | null> {
    ...
    const exact = await this.pool.query(`SELECT ${CHAR_COLS} FROM ipio_woc_characters WHERE name = $1 AND realm = $2`, [name, this.realm]);
    if (exact.rows[0]) return exact.rows[0] as CharInfo;
    const ci = await this.pool.query(`SELECT ${CHAR_COLS} FROM ipio_woc_characters WHERE lower(name) = lower($1) AND realm = $2 LIMIT 2`, [name, this.realm]);
    return ci.rows.length === 1 ? (ci.rows[0] as CharInfo) : null;
  }

  async getCharacter(id: number): Promise<CharInfo | null> {
    const res = await this.pool.query(`SELECT ${CHAR_COLS} FROM ipio_woc_characters WHERE id = $1 AND realm = $2`, [id, this.realm]);
  ...
  async removeFriend(charId: number, friendId: number): Promise<void> {
    await this.pool.query('DELETE FROM ipio_woc_friendships WHERE character_id = $1 AND friend_id = $2', [charId, friendId]);
  }
  ...
  async whoFriended(charId: number): Promise<number[]> {
    const res = await this.pool.query('SELECT character_id FROM ipio_woc_friendships WHERE friend_id = $1', [charId]);
  ...
  async removeBlock(charId: number, blockedId: number): Promise<void> {
    await this.pool.query('DELETE FROM ipio_woc_blocks WHERE character_id = $1 AND blocked_id = $2', [charId, blockedId]);
  }
  ...
  async blockedIds(charId: number): Promise<number[]> {
    const res = await this.pool.query('SELECT blocked_id FROM ipio_woc_blocks WHERE character_id = $1', [charId]);
  ...
  async deleteGuild(id: number): Promise<void> {
    await this.pool.query('DELETE FROM ipio_woc_guilds WHERE id = $1', [id]);
  }
  ...
  async removeGuildMember(charId: number): Promise<void> {
    await this.pool.query('DELETE FROM ipio_woc_guild_members WHERE character_id = $1', [charId]);
  }

  async setGuildRank(charId: number, rank: GuildRank): Promise<void> {
    await this.pool.query('UPDATE ipio_woc_guild_members SET rank = $2 WHERE character_id = $1', [charId, rank]);
  }
